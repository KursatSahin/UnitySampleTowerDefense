...
namespace Utils
{
    public class EventManager
    {
        ...
        public void Subscribe(string eventName, Action<object> callback, int priority = Priority.Low, bool autoDestroy = false)
        {
            lock (_locker)
            {
        ...
        private void UnsubscribeIfAutoDestroyEnabled(string eventName, List<Handler> handlers)
        {
            foreach (var handler in handlers)
            {
                if (handler.AutoDestroy)
                    Unsubscribe(eventName, handler.Method);
            }
        }

        public void Notify(string eventName, object data = null, bool debug = false)
        {
            if (debug)  
            ...
            UnsubscribeIfAutoDestroyEnabled(eventName, notifiedHandlers);
        }
